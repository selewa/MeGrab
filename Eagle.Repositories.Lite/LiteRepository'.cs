...
namespace Eagle.Repositories.Lite
{
    ...
    public class LiteRepository<TAggregateRoot, TIdentityKey> : Repository<TAggregateRoot, TIdentityKey>
        where TAggregateRoot : class, IAggregateRoot<TIdentityKey>, new()
    {
        private ILiteRepositoryContext liteRepositoryContext;

        public LiteRepository(IRepositoryContext repositoryContext)
        ...
        {
            TAggregateRoot aggregateRootToBeDeleted = this.DoFind(new ExpressionSpecification<TAggregateRoot>(a => a.Id.Equals(id)));
