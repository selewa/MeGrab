...
namespace Eagle.Domain.Repositories
{
    public abstract class RepositoryContext : DisposableObject, IRepositoryContext
    {
        ...
        public IRepository<TAggregateRoot, TIdentityKey> GetRepository<TAggregateRoot, TIdentityKey>() 
            where TAggregateRoot : class, IAggregateRoot<TIdentityKey>, new()
        {
            lock (lockObject)
            {
                ...
                if (!repositoryCaches.ContainsKey(entityType))
                {
                    IRepository<TAggregateRoot, TIdentityKey> repository = this.CreateRepository<TAggregateRoot, TIdentityKey>();

                    repositoryCaches.Add(entityType, repository);
                }

                return (IRepository<TAggregateRoot, TIdentityKey>)repositoryCaches[entityType];
            }
        }
        ...
        protected abstract IRepository<TAggregateRoot, TIdentityKey> CreateRepository<TAggregateRoot, TIdentityKey>()
            where TAggregateRoot : class, IAggregateRoot<TIdentityKey>, new();
    }
